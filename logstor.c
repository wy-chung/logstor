...
*/
struct _seg {
	char seg_data[SECTORS_PER_SEG - 1][SECTOR_SIZE];
	struct {
		uint32_t rm[SECTORS_PER_SEG - 1];	// reverse map
		// reverse map SECTORS_PER_SEG - 1 is not used so we store something here
		uint16_t gen;  // sequence number. used for redo after system crash
		uint16_t alloc_p; // allocate sector at this location
	} sum;

	// below are not stored on disk
	uint32_t sega; // the segment address of the segment summary
	unsigned live_count;		
	TAILQ_ENTRY(_seg) queue;
};

_Static_assert(offsetof(struct _seg, sega) == SEG_SIZE,
...
struct g_logstor_softc {
	struct _seg seg_cold;// segment summary for the cold segment
	struct _seg seg_hot;// segment summary for the hot segment
	
	TAILQ_HEAD(, _seg) cc_head; // clean candidate
	struct _seg clean_candidate[CLEAN_WINDOW];
...
static int _logstor_write(uint32_t ba, char *data, int size, struct _seg *seg);
static int _logstor_write_one(uint32_t ba, char *data, struct _seg *seg);
static void seg_alloc(struct _seg *seg);
static void seg_reclaim_init(struct _seg *seg);
...
static void seg_write(struct _seg *seg);
...
static void fbuf_flush(struct _fbuf *buf, struct _seg *seg);
...
logstor_open(void)
{
	...
	seg_alloc(&sc.seg_cold);
	seg_alloc(&sc.seg_hot);
...
logstor_close(void)
{
	...
	seg_write(&sc.seg_cold);
	seg_write(&sc.seg_hot);
...
logstor_write(off_t offset, void *data, off_t length)
{
	...
	if (size == 1) {
		error = _logstor_write_one(ba, data, &sc.seg_hot);
	} else {
		error = _logstor_write(ba, data, size, &sc.seg_hot);
	}
	return error;
}
...
{
	return _logstor_write(ba, data, 1, &sc.seg_hot);
}
...
*/
static int
_logstor_write(uint32_t ba, char *data, int size, struct _seg *seg)
{
	...
	MY_ASSERT(ba < sc.superblock.max_block_cnt);
	MY_ASSERT(seg->sum.alloc_p < SEG_SUM_OFF);

	sec_remain = size;
	while (sec_remain > 0) {
		sec_free = SEG_SUM_OFF - seg->sum.alloc_p;
		...
		for (i = 0; i < count; i++)
			seg->sum.rm[seg->sum.alloc_p++] = ba + i;

		if (seg->sum.alloc_p == SEG_SUM_OFF)
...
static int
_logstor_write_one(uint32_t ba, char *data, struct _seg *seg)
{
	uint32_t sa;	// sector address
...
Output:
  Store the segment address into @seg->sega
  Initialize @seg->sum.alloc_p to 0
*/
static void
seg_alloc(struct _seg *seg)
{
	uint32_t sega;
#if defined(MY_DEBUG)
	uint32_t sega_cold, sega_hot;

	sega_cold = sc.seg_cold.sega;
	sega_hot = sc.seg_hot.sega;
#endif
...
again:
	...
	MY_ASSERT(sc.superblock.seg_alloc_p != sc.superblock.seg_reclaim_p);
...
*/
static void
seg_reclaim_init(struct _seg *seg)
{
	uint32_t sega;
	uint32_t sega_cold, sega_hot;
...
/*
  Input:  seg->sega, segment's sector address
  Output: seg->live_count
*/
static void
seg_live_count(struct _seg *seg)
{
	int	i;
	uint32_t ba;
	uint32_t seg_sa;
	...
	seg_sa = sega2sa(seg->sega);
	for (i = 0; i < seg->sum.alloc_p; i++)
	{
		ba = seg->sum.rm[i];	// get the block address from reverse map
		if (IS_META_ADDR(ba)) {
			if (fbuf_ma2sa((union meta_addr)ba) == seg_sa + i) { // live metadata
				buf = fbuf_search((union meta_addr)ba);
				if (buf == NULL || !buf->modified)
					live_count++;
			}
		} else {
...
cleaner(void)
{
	struct _seg *seg, *seg_to_clean, *seg_prev_head;
...
//printf("\n%s >>>\n", __func__);
	...
	for (;;) {
		...
		seg = NULL; // the head of queue has not been processed
clean:
...
static uint32_t
fbuf_write(struct _fbuf *buf, struct _seg *seg)
{
	uint32_t	sa;	// sector address
	...
	seg->sum.rm[seg->sum.alloc_p++] = buf->ma.uint32;

	if (seg->sum.alloc_p == SEG_SUM_OFF) { // current segment is full
		seg_write(seg);
		seg_alloc(seg);
...
*/
static void
fbuf_flush(struct _fbuf *buf, struct _seg *seg)
{
...
logstor_sa2ba(uint32_t sa)
{
	...
	return (sc.seg_sum_cache.sum.rm[seg_off]);
}
